...
import { TechSupportEstimateForm } from "@domains/estimate/components";
import { ITechSupportEstimateForm } from "@domains/estimate/components/tech-support-estimate-form/TechSupportEstimateForm.interface";
...
const TechSupportEstimateEdit: NextPage = () => {
    ...
    const {
        ...
        watch,
        formState: { errors },
    } = useForm<ITechSupportEstimateForm.IForm>();
    ...
    const {
        ...
        [`get original tech support estimate data of ${id}`],
    ...
    const { data: editedData, mutate: editEstiamteMutate } = useMutation(
        [`edit tech support estimate of ID ${id}`],
        () =>
            ...
            }),
        {
            onSuccess(data, variables, context) {
                router.push(
                    `${PATH.TECHSUPPORT.ESTIMATE.MAIN}/${data.editEstimate.estimate.id}`
                );
            },
        }
    );
    ...
    return (
        <BasicTemplate>
            <PageTitle
                title="기술지원 견적 수정"
                isVisible={false}
                path={`${PATH.TECHSUPPORT.ESTIMATE.MAIN}/${id}`}
            />
            <TechSupportEstimateForm
                register={register}
                control={control}
                errors={errors}
                watch={watch}
...
export default TechSupportEstimateEdit;
