...
import PageSectionTitle from "@common/components/page-section-title/PageSectionTitle.impl";
...
import SalesStatusChart from "@domains/sales/components/sales-status-chart/SalesStatusChart.impl";
import SalesStatusGoal from "@domains/sales/components/sales-status-goal/SalesStatusGoal.impl";
...
const LicenseSalesStats: NextPage = () => {
    // 현재 년도
    const nowYear = new Date().getFullYear();

    const { data: commonType, isSuccess: isCommonTypeSuccess } = useQuery(
        ["getCommonCode"],
        () =>
    ...
    const {
        ...
        () =>
            contractService.getAllContract({
                where: {
                    type: { value: { _in: [ProductType.LICENSE] } },
                    // project: {
                    //     is_canceled: { _eq: false },
                    // },
    ...
    const { refetch: goalRefetch } = useQuery(
        ...
        {
            enabled: isCommonTypeSuccess,
            onSuccess(data) {
                // console.log(data);
                let goalRateData: number[] = [];
                salesTotal.forEach(item => {
                    if (data.sales_status_by_pk) {
                        goalRateData.push(
                            Math.floor(
                                (item.total /
                                    moneyToNumber(
            ...
            <BasicTemplate>
                ...
                <div id="contents-section">
                    <div id="sales-status-chart-section" className="section">
                        ...
                        <SalesStatusChart
                            common_type={ProductType.LICENSE}
                            salesData={salesData}
...
export default LicenseSalesStats;
const rootStyle = css`
    #page-info-section {
        #page-path {
            display: flex;
