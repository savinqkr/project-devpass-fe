...
const LicenseEstimateEdit: NextPage = () => {
    ...
    const {
        ...
        [`get original license estimate data of ${id}`],
    ...
    const { data: editedData, mutate: editEstiamteMutate } = useMutation(
            ...
            }),
        {
            onSuccess(data, variables, context) {
                router.push(
                    `${PATH.LICENSE.ESTIMATE.MAIN}/${data.editEstimate.estimate.id}`
                );
            },
        }
    );
    ...
    return (
        <BasicTemplate>
            <PageTitle
                title="라이선스 견적 수정"
                isVisible={false}
                path={`${PATH.LICENSE.ESTIMATE.MAIN}/${id}`}
            />
            <LicenseEstimateForm
                register={register}
                control={control}
...
export default LicenseEstimateEdit;
