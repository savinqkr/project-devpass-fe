...
import { CustomizeEstimateDetail } from "@domains/estimate/components";
import { IconButton } from "@mui/material";
import { MdOutlineDelete, MdOutlineEdit } from "react-icons/md";

const CustomizeEstimateDetailPage: NextPage = () => {
    ...
    // 최종 견적 확정  ( MUTATION )
    // ---------------------------------------------
    const { data: finalEstimate, mutate: setFinalEstimateMutate } = useMutation(
    ...
    const { data: deletedEstiamte, mutate: deleteEstimateMutate } = useMutation(
        ...
        {
            onSuccess(data, variables, context) {
                router.push(PATH.CUSTOMIZE.ESTIMATE.MAIN);
            },
        }
    );
    ...
    return (
        <BasicTemplate>
            <div css={rootStyle}>
                <PageTitle
                    title="커스터마이징 개발 견적 상세"
                    isVisible={true}
                    path={PATH.CUSTOMIZE.ESTIMATE.MAIN}
                />
                <div className="group">
                    <p className="menu-info">
                        {"커스터마이징 개발 > 커스터마이징 개발 견적 > "}
                            ...
                            !estimateData?.estimate_by_pk.is_final && (
                                <>
                                    ...
                                    <IconButton
                                        ...
                                        onClick={() =>
                                            router.push(
                                                `${PATH.CUSTOMIZE.ESTIMATE.EDIT}/${id}`
                                            )
                                        }
                                        disabled={
                                            estimateData?.estimate_by_pk.project
...
export default CustomizeEstimateDetailPage;
