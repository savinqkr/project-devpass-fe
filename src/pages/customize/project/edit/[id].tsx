...
const CustomizeProjectEdit: NextPage = () => {
    ...
    // 사업 정보 수정 ( MUTATION )
    // ---------------------------------------------------
    const { mutate: editCustomizeProjectMutate } = useMutation(
        [`edit customize proejct of id ${projectData?.project_by_pk.id}`],
        () =>
            ...
            }),
        {
            onSuccess(data) {
                router.push(
                    `${PATH.CUSTOMIZE.PROJECT.MAIN}/${data.editProject.project.id}`
                );
            },
        }
    );
    ...
    return (
        <BasicTemplate>
            <PageTitle
                title="커스터마이징 개발 사업 수정"
                isVisible={false}
                path={`${PATH.CUSTOMIZE.PROJECT.MAIN}/${id}`}
            />
...
export default CustomizeProjectEdit;
