...
import { ITechSupportEstimateForm } from "./TechSupportEstimateForm.interface";
import VTechSupportEstimateForm from "./TechSupportEstimateForm.view";
...
const TechSupportEstimateForm: React.FC<
    ITechSupportEstimateForm.IProps
> = props => {
    const {
    ...
    const {
        ...
        ["get selected estimate by pk for tech support estimate"],
        () =>
    ...
    const { data: clientData } = useQuery(
        ["get client options for tech support estimate"],
        () =>
    ...
    const { data: projectOptions } = useQuery(
        [
            "get project options by selected client for tech support estimate",
        ...
        () =>
            projectService.getProjectByConditions({
                where: {
                    type: {
                        code: {
                            _eq: techSupportCode?.common_code[0].code || 0,
                        },
                    },
                    client_id: {
                        _eq: parseInt(
        ...
        {
            enabled:
                !!techSupportCode?.common_code[0].code &&
    ...
    const { data: employeeOpions } = useQuery(
        [
            "get employee options by selected project for tech support estimate",
            watch("project_id"),
            isAutoCompleteActivated,
        ],
        () =>
    ...
    const { data: tailData } = useQuery(
        ["get tech support tail options for tech support estimate"],
        () =>
    ...
    const { data: products } = useQuery(
        ["get products by type for tech support estimate"],
        () =>
    ...
    const { data: productPurpose } = useQuery(
        ["get product purpose for tech support estimate"],
        () =>
            codeService.getCommonCode({
                where: {
    ...
    const { data: productClass } = useQuery(
        ["get product class for tech support estimate"],
        () =>
            codeService.getCommonCode({
                where: {
    ...
    const handleClickAddRow = () => {
        setIsAutoCompleteActivated(true); // 자동 입력 활성화
        const newRow: ITechSupportEstimateForm.IDetailForm = {
            id: rows.length + 1,
            type: "JOB-PaSS 기술지원",
            purpose: "선택해주세요",
            class: "선택해주세요",
...
export default TechSupportEstimateForm;
