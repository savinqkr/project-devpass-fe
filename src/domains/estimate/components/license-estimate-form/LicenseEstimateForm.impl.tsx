...
const LicenseEstimateForm: React.FC<ILicenseEstimateForm.IProps> = props => {
    ...
    const {
        ...
        ["get selected estimate by pk for license estimate"],
        () =>
    ...
    // 세부내역 TABLE
    // ---------------------------------------------------------------------------------
    // 라이선스 제품 옵션 ( TYPE : 라이선스 / 기술지원 )
    const { data: products } = useQuery(
        ["get products by type for license estimate"],
        () =>
    ...
    const { data: productPurpose } = useQuery(
        ["get product purpose for license estimate"],
        () =>
            codeService.getCommonCode({
                where: {
    ...
    const { data: productClass } = useQuery(
        ["get product class for license estimate"],
        () =>
            codeService.getCommonCode({
                where: {
