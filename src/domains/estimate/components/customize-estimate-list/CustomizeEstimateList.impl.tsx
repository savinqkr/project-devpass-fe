...
import { ICustomizeEstimateList } from "./CustomizeEstimateList.interface";
import VTechSupportEstimateList from "./CustomizeEstimateList.view";
...
const CustomizeEstimateList: React.FC<
    ICustomizeEstimateList.IProps
> = props => {
    ...
    const { setValue, getValues, handleSubmit, control, register } =
        useForm<ICustomizeEstimateList.ISearch>();
    ...
    // 커스터마이징 개발 견적 조회 ( QUERY )
    const {
        data,
        isLoading,
        refetch: fetchAllEstimates,
    } = useQuery(
        ["get all customize dev estimates", showDeleted],
        () =>
            estimateService.getAllEstimates({
                where: {
                    type: { code: { _eq: customizeCode!.common_code[0].code } },
                    _and: [
        ...
        {
            enabled: !!customizeCode?.common_code[0].code,
            onSuccess(data) {
    ...
    const onClickRow = (pk: string) => {
        router.push(`${PATH.CUSTOMIZE.ESTIMATE.MAIN}/${pk}`);
    };
...
export default CustomizeEstimateList;
