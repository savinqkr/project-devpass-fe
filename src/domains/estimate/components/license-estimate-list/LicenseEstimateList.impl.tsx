...
import VLicenseEstimateList from "./LicenseEstimateList.view";
...
const LicenseEstimateList: React.FC<ILicenseEstimateList.IProps> = props => {
    const router = useRouter();

    const loginUser = useRecoilValue(loginState);

    const { setValue, getValues, handleSubmit, control, register } =
        useForm<ILicenseEstimateList.ISearch>();
    ...
    const {
        ...
        ["get all license estimates", showDeleted],
        () =>
            estimateService.getAllEstimates({
                where: {
                    type: { code: { _eq: licenseCode!.common_code[0].code } },
                    _and: [
...
export default LicenseEstimateList;
